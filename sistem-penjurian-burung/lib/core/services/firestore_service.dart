...
Future<UserModel> getUserData(String uid) async {
  ...
  Stream<List<SesiModel>> getAssignedSesi(String juriId) {
    debugPrint(
      "[FirestoreService] Mencari sesi untuk Juri UID: $juriId (Query Sederhana)",
    );
    ...
    return _firestore
        ...
        .map((snapshot) {
          debugPrint(
            "[FirestoreService] Query berhasil, ditemukan ${snapshot.docs.length} sesi.",
          );
          return snapshot.docs
              .map((doc) => SesiModel.fromFirestore(doc))
              .toList();
  ...
  Stream<List<PenilaianModel>> getMyScoresForSesi(
    String sesiId,
    String juriId,
  ) {
    return _firestore
        ...
        .map(
          (snapshot) =>
              snapshot.docs
                  .map(
                    (doc) => PenilaianModel.fromFirestore(doc),
                  ) // Anda perlu membuat factory constructor ini
                  .toList(),
        );
  }
  ...
  Future<void> undiNomorGantangan(String eventId, String sesiId) async {
    try {
      ...
      final List<int> nomorGantanganTersedia = List.generate(
        jumlahPeserta,
        (i) => i + 1,
      );
  ...
  Future<void> finalisasiHasilSesi(SesiModel sesi) async {
    try {
      ...
      final pendaftaranSnapshot =
          await _firestore
              .collection('pendaftaran')
              .where('sesiId', isEqualTo: sesi.id)
              .get();
      final penilaianSnapshot =
          await _firestore
              .collection('penilaian')
              .where('sesiId', isEqualTo: sesi.id)
              .get();
      ...
      final pendaftaranList =
          pendaftaranSnapshot.docs
              .map((doc) => PendaftaranModel.fromFirestore(doc))
              .toList();
      final allScores =
          penilaianSnapshot.docs
              .map((doc) => PenilaianModel.fromFirestore(doc))
              .toList();
  ...
  Stream<HasilAkhirModel?> getHasilAkhir(String sesiId) {
    return _firestore.collection('hasil_akhir').doc(sesiId).snapshots().map((
      snapshot,
    ) {
      if (snapshot.exists) {
  ...
  Stream<List<PendaftaranModel>> getPaymentHistory() {
    return _firestore
        ...
        .where(
          'status',
          whereIn: [StatusPembayaran.lunas.name, StatusPembayaran.ditolak.name],
        )
...
final myScoresProvider = StreamProvider.family<List<PenilaianModel>, String>((
  ref,
  sesiId,
) {
  final user = ref.watch(authStateChangesProvider).value;
  if (user != null) {
    return ref
        ...
        .getMyScoresForSesi(sesiId, user.uid);
  }
  return Stream.value([]);
});
...
final sesiStreamProvider = StreamProvider.family<List<SesiModel>, String>((
  ref,
  eventId,
) {
...
final leaderboardProvider = StreamProvider.family<List<PenilaianModel>, String>(
  (ref, sesiId) {
...
final hasilAkhirProvider = StreamProvider.family<HasilAkhirModel?, String>((
  ref,
  sesiId,
) {
